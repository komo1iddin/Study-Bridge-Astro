...
import { featureTranslations } from '../src/i18n/features.js';
import { translations } from '../src/i18n/translations/new-index.js';
import { getMissingTranslations } from '../src/i18n/translationUtils.js';
 ...
 */
function auditFeatureTranslations() {
  const report = [];
  
  const features = Object.keys(featureTranslations);
  ...
  features.forEach(feature => {
    try {
      ...
      defaultLangKeys.forEach(key => {
        ...
        const enValue = key.split('.').reduce((obj, k) => obj && obj[k], featureTranslations[feature].en);
        if (enValue === undefined) missingEn.push(key);
        
        // Check Russian translations
        const ruValue = key.split('.').reduce((obj, k) => obj && obj[k], featureTranslations[feature].ru);
